...
postgres_password="postgres1122"
max_whats="999"
max_user="999"
frontend_port="3435"
backend_port="4142"
...
# Función para verificar permisos de PostgreSQL
verify_postgres_permissions() {
    log_message "INFO" "Verificando permisos de PostgreSQL..."
    
    # Verificar si PostgreSQL está instalado
    if ! command -v psql &> /dev/null; then
        log_message "INFO" "PostgreSQL no está instalado aún, se instalará más adelante"
        return 0
    fi
    
    # Verificar si se puede conectar como postgres
    if ! sudo -u postgres psql -c "SELECT 1;" > /dev/null 2>&1; then
        log_message "WARNING" "⚠️ No se puede conectar a PostgreSQL como usuario postgres"
        
        # Intentar configurar permisos
        log_message "INFO" "Configurando permisos de PostgreSQL..."
        
        # Verificar que el usuario postgres existe
        if ! id postgres &> /dev/null; then
            log_message "ERROR" "❌ Usuario postgres no existe"
            return 1
        fi
        
        # Verificar directorio de datos de PostgreSQL
        if ! sudo test -d /var/lib/postgresql; then
            log_message "ERROR" "❌ Directorio de datos de PostgreSQL no existe"
            return 1
        fi
        
        log_message "WARNING" "⚠️ PostgreSQL puede requerir configuración manual"
    else
        log_message "SUCCESS" "✅ Permisos de PostgreSQL verificados"
    fi
}
...
get_link_git() {
    ...
    default_link="https://github.com/leopoldohuacasiv/watoolx.git"
...
show_configuration_summary() {
    ...
    echo -e "${GRAY_LIGHT}• Contraseña PostgreSQL: postgres1122${NC}"
...
backend_redis_create() {
    ...
    if ! command -v psql &> /dev/null; then
        log_message "INFO" "Instalando PostgreSQL..."
        if ! sudo apt-get install -y postgresql postgresql-contrib; then
            register_error "No se pudo instalar PostgreSQL"
            return 1
        fi
    fi
    ...
    if ! docker run --name redis-${instancia_add} -p ${redis_port}:6379 --restart always --detach redis redis-server --requirepass ${postgres_password}; then
    ...
    if ! sudo -u postgres createdb ${instancia_add} 2>/dev/null; then
        log_message "WARNING" "La base de datos ${instancia_add} ya existe"
    fi

    # Crear usuario PostgreSQL
    if ! sudo -u postgres psql -c "CREATE USER ${instancia_add} SUPERUSER INHERIT CREATEDB CREATEROLE;" 2>/dev/null; then
        log_message "WARNING" "El usuario ${instancia_add} ya existe"
    fi

    # Establecer contraseña
    if ! sudo -u postgres psql -c "ALTER USER ${instancia_add} PASSWORD '${postgres_password}';"; then
        register_error "No se pudo establecer la contraseña del usuario PostgreSQL"
        return 1
    fi

    # Instalar extensión uuid-ossp para PostgreSQL
    if ! sudo -u postgres psql -d ${instancia_add} -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";"; then
        register_error "No se pudo instalar la extensión uuid-ossp"
        return 1
    fi
    ...
    log_message "SUCCESS" "✅ Redis y PostgreSQL configurados correctamente"
    sleep 2
    return 0
}
...
# Redis
REDIS_URI=redis://:${postgres_password}@127.0.0.1:${redis_port}
...
backend_db_migrate() {
    ...
    table_count=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';" 2>/dev/null | tr -d ' ')
...
backend_db_seed() {
    ...
    if ! sudo -u postgres psql -d ${instancia_add} -c "SELECT 1 FROM \"Companies\" LIMIT 1;" > /dev/null 2>&1; then
        log_message "ERROR" "La tabla Companies no existe"
        register_error "Tabla Companies no encontrada"
        return 1
    fi
    
    if ! sudo -u postgres psql -d ${instancia_add} -c "SELECT 1 FROM \"Users\" LIMIT 1;" > /dev/null 2>&1; then
        log_message "ERROR" "La tabla Users no existe"
        register_error "Tabla Users no encontrada"
        return 1
    fi
    
    if ! sudo -u postgres psql -d ${instancia_add} -c "SELECT 1 FROM \"Plans\" LIMIT 1;" > /dev/null 2>&1; then
    ...
    existing_user=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Users\" WHERE email = 'admin@admin.com';" 2>/dev/null | tr -d ' ')
    existing_company=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Companies\" WHERE name IN ('Empresa 1', '${instancia_add}');" 2>/dev/null | tr -d ' ')
    existing_plan=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Plans\" WHERE name = 'Plano 1';" 2>/dev/null | tr -d ' ')
    ...
    if npm run db:seed > /dev/null 2>&1; then
        ...
        user_count=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Users\";" 2>/dev/null | tr -d ' ')
    ...
    if [ "$existing_plan" -gt 0 ]; then
        ...
        if sudo -u postgres psql -d ${instancia_add} -c "BEGIN; INSERT INTO \"Plans\" (name, users, connections, queues, value, createdAt, updatedAt) VALUES ('Plano 1', 10, 10, 10, 30, NOW(), NOW()); COMMIT;" 2>/dev/null; then
            ...
            sleep 2
            plan_check=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Plans\" WHERE name = 'Plano 1';" 2>/dev/null | tr -d ' ')
    ...
    plan_id=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT id FROM \"Plans\" WHERE name = 'Plano 1' ORDER BY id DESC LIMIT 1;" 2>/dev/null | tr -d ' ')
    
    if [ -z "$plan_id" ] || [ "$plan_id" = "" ]; then
        # Intentar obtener cualquier plan existente
        plan_id=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT id FROM \"Plans\" ORDER BY id ASC LIMIT 1;" 2>/dev/null | tr -d ' ')
        if [ -z "$plan_id" ] || [ "$plan_id" = "" ]; then
            plan_id=1
    ...
    existing_company=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Companies\" WHERE name IN ('${instancia_add}', 'Empresa 1');" 2>/dev/null | tr -d ' ')
    ...
    if [ "$existing_company" -gt 0 ]; then
        ...
        if sudo -u postgres psql -d ${instancia_add} -c "BEGIN; INSERT INTO \"Companies\" (name, phone, email, status, planId, dueDate, recurrence, createdAt, updatedAt) VALUES ('Empresa 1', '5511999999999', 'admin@admin.com', true, $plan_id, NOW() + INTERVAL '30 days', 'MONTHLY', NOW(), NOW()); COMMIT;" 2>/dev/null; then
            ...
            sleep 2
            company_check=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Companies\" WHERE name IN ('${instancia_add}', 'Empresa 1');" 2>/dev/null | tr -d ' ')
    ...
    company_id=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT id FROM \"Companies\" WHERE name IN ('${instancia_add}', 'Empresa 1') ORDER BY id DESC LIMIT 1;" 2>/dev/null | tr -d ' ')
    
    if [ -z "$company_id" ] || [ "$company_id" = "" ]; then
        # Intentar obtener cualquier empresa existente
        company_id=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT id FROM \"Companies\" ORDER BY id ASC LIMIT 1;" 2>/dev/null | tr -d ' ')
        if [ -z "$company_id" ] || [ "$company_id" = "" ]; then
            company_id=1
    ...
    existing_admin=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Users\" WHERE email = 'admin@admin.com';" 2>/dev/null | tr -d ' ')
    ...
    if [ "$existing_admin" -gt 0 ]; then
        ...
        if sudo -u postgres psql -d ${instancia_add} -c "BEGIN; INSERT INTO \"Users\" (name, email, \"passwordHash\", profile, \"companyId\", \"createdAt\", \"updatedAt\", super, \"allTicket\") VALUES ('Admin', 'admin@admin.com', '\$2a\$08\$08TgjZAGBqkI9Uk.T8lKvO4vyE2JQoG8XeJdmY9pIqR2SR3aJqKq', 'admin', $company_id, NOW(), NOW(), true, 'enabled'); COMMIT;" 2>/dev/null; then
            ...
            sleep 2
            admin_check=$(sudo -u postgres psql -d ${instancia_add} -t -c "SELECT COUNT(*) FROM \"Users\" WHERE email = 'admin@admin.com';" 2>/dev/null | tr -d ' ')
...
verify_services_status() {
    ...
    if sudo systemctl is-active --quiet postgresql; then
        log_message "SUCCESS" "✅ PostgreSQL ejecutándose"
    else
        log_message "ERROR" "❌ PostgreSQL no está ejecutándose"
    fi
